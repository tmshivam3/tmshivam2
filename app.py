...
def get_random_font():
    ...
    font_folders = [f for f in os.listdir(fonts_dir) if os.path.isdir(os.path.join(fonts_dir, f))]
...
def analyze_image_for_text(img):
    ...
    text_position = (img.width // 2, img.height // 3)
    
    img_height, img_width = img_np.shape[:2]
    base_size = min(img_width, img_height) // 3
    
    return {
        'text_color': text_color,
        'position': text_position,
        'font_size': min(max(base_size, 80), 150)
    }
...
def create_variant(original_img, settings, text_effect=None):
    ...
    if settings['show_text']:
        ...
        analysis['font_size'] = max(analysis['font_size'], settings.get('main_size', 100))
    ...
    if settings['show_wish']:
        font_wish = get_random_font()
        font_wish.size = settings['wish_size']
        ...
        if settings['show_text']:
            ...
            wish_y = analysis['position'][1] + analysis['font_size'] + 30
        else:
            max_wish_x = max(20, img.width - wish_width - 20)
            wish_x = random.randint(20, max_wish_x) if max_wish_x > 20 else 20
    ...
    if settings['show_date']:
        font_date = get_random_font()
        font_date.size = settings['date_size']
        ...
        date_y = max(20, img.height - date_height - 30)
...
with st.sidebar:
    ...
    if show_text:
        main_size = st.slider("Main Text Size", 60, 200, 120)
    
    show_wish = st.checkbox("Show Wish", value=True)
    if show_wish:
        wish_size = st.slider("Wish Text Size", 40, 200, 80)
    
    show_date = st.checkbox("Show Date", value=False)
    if show_date:
        date_size = st.slider("Date Text Size", 30, 200, 50)
    ...
    if uploaded_images:
        with st.spinner("Processing images..."):
            ...
            settings = {
                ...
                'show_text': show_text,
                'main_size': main_size if show_text else 100,
                'show_wish': show_wish,
                'wish_size': wish_size if show_wish else 80,
                'show_date': show_date,
                'show_day': show_day if show_date else False,
                'date_size': date_size if show_date else 50,
